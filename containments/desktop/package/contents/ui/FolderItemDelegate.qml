...
Item {
    ...
    Component {
        ...
        Item {
            ...
            property Item labelArea: frameLoader.textShadow ? frameLoader.textShadow : label
            ...
            onSelectedChanged: {
                if (selected && !blank) {
                    frameLoader.grabToImage(function(result) {
                        dir.addItemDragImage(positioner.map(index), main.x + frameLoaderx, main.y + frameLoader.y, frameLoader.width, frameLoader.height, result.image);
                    });
                }
            }
