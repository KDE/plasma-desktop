...
bool XkbHelper::initializeKeyboardLayouts()
{
	...
	if( config.configureLayouts ) {
		QStringList layouts;
		QStringList variants;
		QList<LayoutUnit> defaultLayouts = config.getLayouts();
		foreach (const LayoutUnit& layoutUnit, defaultLayouts) {
<<<<<<< HEAD:kcms/keyboard/xkb_helper.cpp
            layouts.append(layoutUnit.layout());
            variants.append(layoutUnit.variant());
=======
            if (layoutUnit.technology == "xkb") {
                layouts.append(layoutUnit.layout);
                variants.append(layoutUnit.variant);
            }
>>>>>>> Port Keyboard input modules and Expand scope to cover input method configuration:kcms/keyboard/daemon/xkb_helper.cpp
		}
