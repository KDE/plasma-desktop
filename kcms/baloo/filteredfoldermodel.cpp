...
void FilteredFolderModel::updateDirectoryList()
{
    ...
    std::sort(m_folderList.begin(), m_folderList.end(), [](const FolderInfo &a, const FolderInfo &b) { return a.url < b.url; });
...
void FilteredFolderModel::addFolder(const QString &url, const bool included = false)
{
    ...
    auto it = std::find_if(m_folderList.begin(), m_folderList.end(), [nUrl](const FolderInfo &folder) { return folder.url == nUrl; });
...
{
    return {{Url, "url"}, {Folder, "folder"}, {EnableIndex, "enableIndex"}, {Deletable, "deletable"}, {Qt::DecorationRole, "decoration"}};
}
