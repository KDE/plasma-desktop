...
void DevicesModel::resetModel()
{
    ...
    if (reply.isValid()) {
        qCDebug(KCM_TOUCHSCREEN) << "Devices list received successfully from KWin.";
        devicesSysNames = reply.toStringList();
    } else {
        qCCritical(KCM_TOUCHSCREEN) << "Error on receiving device list from KWin.";
        return;
    }
...
void DevicesModel::addDevice(const QString &sysName, bool tellModel)
{
    ...
    if (reply.isValid() && reply.toBool()) {
        ...
        qCDebug(KCM_TOUCHSCREEN).nospace() << "Device connected: " << dev->name() << " (" << dev->sysName() << ")";
...
void DevicesModel::onDeviceRemoved(const QString &sysName)
{
    ...
    InputDevice *dev = it->get();
    qCDebug(KCM_TOUCHSCREEN).nospace() << "Device disconnected: " << dev->name() << " (" << dev->sysName() << ")";
