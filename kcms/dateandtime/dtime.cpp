...
void Dtime::currentZone()
{
    ...
    if (localZone.abbreviations().isEmpty()) {
        ...
        m_local->setText(i18nc("%1 is name of time zone, %2 is its abbreviation", "Current local time zone: %1 (%2)", K4TimeZoneWidget::displayName(localZone), QString::fromUtf8(localZone.abbreviations().first())));
    }
}
