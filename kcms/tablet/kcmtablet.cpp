...
public:
    enum CustomRoles { NameRole = Qt::UserRole, TextRole, ValueRole };

    StylusButtonsModel()
    {
        setItemRoleNames({{NameRole, "name"}, {TextRole, "text"}, {ValueRole, "value"}});
        recalculateItems();
    }
...
private:
    void recalculateItems()
    {
        ...
        for (int i = 0; i < numButtons; i++) {
            ...
            item->setData(i18nc("Stylus pen button", "Pen button %1:", i + 1), TextRole);
