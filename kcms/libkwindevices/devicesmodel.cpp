    ...
    , m_filters(filters)
{
    QDBusConnection::sessionBus()
        .connect(g_kwinService, g_kwinInputDevicePath, g_kwinInputDeviceManagerInterface, u"deviceAdded"_s, this, SLOT(onDeviceAdded(QString)));
    QDBusConnection::sessionBus()
        .connect(g_kwinService, g_kwinInputDevicePath, g_kwinInputDeviceManagerInterface, u"deviceRemoved"_s, this, SLOT(onDeviceRemoved(QString)));
    ...
    for (const QString &sysName : deviceSysNames) {
        ...
        if (!m_filters.isEmpty()) {
            org::kde::KWin::InputDevice deviceIface{g_kwinService, g_kwinInputDevicePath + '/'_L1 + sysName, QDBusConnection::sessionBus()};
