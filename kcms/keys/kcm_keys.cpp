    ...
    : KQuickAddons::ConfigModule(parent, args)
{
    ...
    setAboutData(about);
    m_globalAccelInterface = new KGlobalAccelInterface(QStringLiteral("org.kde.kglobalaccel"), QStringLiteral("/kglobalaccel"), QDBusConnection::sessionBus(), this);
...
void KCMKeys::requestKeySequence(QQuickItem *context, const QModelIndex &index, const QKeySequence &newSequence, const QKeySequence &oldSequence)
{
    ...
    const QString message = isStandardAction
        ? i18nc("%2 is the name of a category inside the 'Common Actions' section", "Shortcut %1 is already assigned to the common %2 action '%3'.\nDo you want to reassign it?", keysString, componentName, actionName)
    ...
    KMessageBox::createKMessageBox(dialog, new QDialogButtonBox(QDialogButtonBox::Yes | QDialogButtonBox::No, dialog), QMessageBox::Question, message, {}, QString(), nullptr, KMessageBox::NoExec);
    dialog->show();
