...
void BaseModel::defaults()
{
    for (int i = 0; i < m_components.size(); ++i) {
        ...
        Q_EMIT dataChanged(index(0, 0, componentIndex), index(m_components[i].actions.size() - 1, 0, componentIndex), {ActiveShortcutsRole, CustomShortcutsRole, IsDefaultRole});
    }
...
QVariant BaseModel::data(const QModelIndex &index, int role) const
{
    ...
    case IsDefaultRole:
        return std::all_of(component.actions.begin(), component.actions.end(), [](const Action &action) { return action.activeShortcuts == action.defaultShortcuts; });
    }
    return QVariant();
}
