...
void StandardShortcutsModel::load()
{
    ...
    for (int i = KStandardShortcut::AccelNone + 1; i < KStandardShortcut::StandardShortcutCount; ++i) {
        ...
        auto listToSet = [](const QList<QKeySequence> &list) { return QSet<QKeySequence> {list.cbegin(), list.cend()}; };
    ...
    std::for_each(m_components.begin(), m_components.end(), [&](Component &c) { std::sort(c.actions.begin(), c.actions.end(), [&](const Action &a1, const Action &a2) { return collator.compare(a1.displayName, a2.displayName) < 0; }); });
    std::sort(m_components.begin(), m_components.end(), [&](Component &c1, Component &c2) { return collator.compare(c1.displayName, c2.displayName) < 0; });
    endResetModel();
}
...
void StandardShortcutsModel::importConfig(const KConfigBase &config)
{
    ...
    for (const auto &key : keys) {
        ...
        auto cat = std::find_if(m_components.begin(), m_components.end(), [&name](const Component &c) { return c.id == name; });
        if (cat == m_components.end()) {
            qCWarning(KCMKEYS) << "No category for standard shortcut" << key;
            continue;
        }
        auto action = std::find_if(cat->actions.begin(), cat->actions.end(), [&](const Action &a) { return a.id == key; });
