...
{
    static const bool isKRunner = KLocalizedString::applicationDomain() == QByteArrayView("krunner");
    static const bool isPlasmashell = KLocalizedString::applicationDomain() == QByteArrayView("plasmashell");
    ...
    for (int i = 0, count = m_filteredModel->rowCount(); i < count; ++i) {
        ...
        if (isKRunner && componentUniqueName == u"org.kde.krunner.desktop") {
            continue;
        } else if (isPlasmashell && componentUniqueName == u"plasmashell") {
            continue;
        }
