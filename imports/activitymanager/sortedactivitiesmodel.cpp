    ...
    , m_activities(new KActivities::Consumer(this))
{
    ...
    connect(KWindowSystem::self(), SIGNAL(windowChanged(WId, NET::Properties, NET::Properties2)), this, SLOT(onWindowChanged(WId, NET::Properties, NET::Properties2)));
}
...
void SortedActivitiesModel::onWindowAdded(WId window)
{
    ...
    for (const auto &activity : activities) {
        if (!m_activitiesWindows[activity].contains(window)) {
            ...
            rowChanged(rowForActivityId(activity), m_activitiesWindows.size() == 1 ? QVector<int> {WindowCount, HasWindows} : QVector<int> {WindowCount});
        }
    }
}
...
void SortedActivitiesModel::onWindowRemoved(WId window)
{
    for (const auto &activity : m_activitiesWindows.keys()) {
        if (m_activitiesWindows[activity].contains(window)) {
            ...
            rowChanged(rowForActivityId(activity), m_activitiesWindows.size() == 0 ? QVector<int> {WindowCount, HasWindows} : QVector<int> {WindowCount});
        }
    }
}
