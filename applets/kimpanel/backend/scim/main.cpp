...
static QVariantList LookupTable2VariantList(const LookupTable &lookup_table)
{
    ...
    result << labels << candidates << attrlist_list << lookup_table.get_current_page_start() << (lookup_table.get_current_page_start() + lookup_table.get_current_page_size() < ((int)lookup_table.number_of_candidates()));
...
static QString Property2String(const Property &prop)
{
    ...
    result =
        QString("%1:%2:%3:%4:%5").arg(QString::fromUtf8(prop.get_key().c_str())).arg(QString::fromUtf8(prop.get_label().c_str())).arg(QString::fromUtf8(prop.get_icon().c_str())).arg(QString::fromUtf8(prop.get_tip().c_str())).arg(state);
    return result;
}
        ...
        if (e->type() == dbus_event_type) {
            ...
            case DBusEvent::SHOW_FACTORY_MENU:
                ...
                Q_FOREACH (const QVariant &v, ev->data()) {
                    ...
                    list_result << Property2String(Property(String(factory_prop_prefix.toUtf8().constData()) + factory_info.uuid, factory_info.name, factory_info.icon, factory_info.lang));
                }
                message << list_result;
...
static void slot_update_lookup_table(const LookupTable &table)
{
    ...
    qApp->postEvent(_dbus_handler, new DBusEvent(DBusEvent::UP_LOOKUPTABLE_CURSOR, QVariantList() << (table.is_cursor_visible() ? table.get_cursor_pos_in_current_page() : -1)));
}
