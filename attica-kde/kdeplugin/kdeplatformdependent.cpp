    ...
    if (accountsManager) {
        ...
        for (const Accounts::AccountId &accountId : accountIds) {
            ...
            if (idToken.isEmpty()) {
                ...
                qCWarning(ATTICA_PLUGIN_LOG) << "We got an OpenDesktop account, but it seems to be lacking the id token. This means an old SignOn OAuth2 plugin was used for logging in. The plugin may have been upgraded in the meantime, "
                                                "but an account created using the old plugin cannot be used, and you must log out and back in again.";
            }
        }
    } else {
